...
/**
 * Provides access to the WishItem database. Since this is not a Content
...
public class WishListDataBase extends SQLiteOpenHelper {
	...
	public void onCreate(SQLiteDatabase db) {
		String[] sql = mContext.getString(R.string.WishListDataBase_Create)
				.split("\n");
		db.beginTransaction();
		try {
		...
		// Uri pic_uri = Uri.parse("android.resource://com.aripio.wishlist/" +
		// R.drawable.book);
	...
	public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
		Log.w(WishList.LOG_TAG, "Upgrading database from version " + oldVersion
				+ " to " + newVersion + ", which will destroy all old data");

		String[] sql = mContext.getString(R.string.WishListDataBase_Upgrade)
				.split("\n");
		db.beginTransaction();
		try {
			// Create tables & test data
	 ...
	 */
	public void addItem(String name, String description, String date,
		...
		String sql = String
				.format(
						...
						name, description, date, store_id, picture_uri, price,
						location, priority);
		try {
	...
	public void editItem(long _id, String name, String description,
		...
		String sql = String.format("UPDATE WishItems " + "SET name = '%s',  "
				+ " description = '%s', " + " create_date = '%s', "
	...
	public void deleteItem(long _id) {
		String sql = String.format("DELETE FROM WishItems "
				+ "WHERE _id = '%d' ", _id);
		try {
			getWritableDatabase().execSQL(sql);
		} catch (SQLException e) {
	...
	public int getItemsCount() {
		...
		try {
			c = getReadableDatabase().rawQuery(
					"SELECT count(*) FROM WishItems", null);
			if (0 >= c.getCount()) {
				return 0;
			}
			c.moveToFirst();
	...
	public static class ItemsCursor extends SQLiteCursor {
		public static enum SortBy {
			name, create_date, price, priority, _id,

		}

		private static final String QUERY = "SELECT _id, name, description, create_date, store_id, picture, price, location, priority "
				+ "FROM WishItems " + "ORDER BY ";
