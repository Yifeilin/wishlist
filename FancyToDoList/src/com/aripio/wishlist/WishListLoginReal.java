	      	...
	      	public void onClick(View viewParam)
	      	{
                ...
                if (c == null || c.getCount() == 0){
                	//user name has not been created, add the user to database
		      		//pop up a window to inform user of successful user creation
		      		//when user clicks "ok", bring user to the main activity
                	long insert_id = mDbHelper.createUser(aUser.getName(), aUser.getPassword());
                	logInMsg.setText("user account is created successfully, please log in");
                	logInMsg.setVisibility(View.VISIBLE);
                }
                
                else{
     	      		//user name has already been created, ask user to use another 
...
//                    		c.getColumnIndexOrThrow(UserDB.KEY_PASSWORD));
                	...
                	logInMsg.setText("user name has been used, try another user name");
                	logInMsg.setVisibility(View.VISIBLE);
      ...
      log_in.setOnClickListener( new OnClickListener()
  	  {
      	public void onClick(View viewParam)
      	{
            ...
            if (c == null || c.getCount() == 0 ){
            	...
            	logInMsg.setText("user name is incorrect, please try again");
            	logInMsg.setVisibility(View.VISIBLE);
            ...
            //user name is correct, check password
            	...
            	String correctPass = c.getString(
                		c.getColumnIndexOrThrow(UserDB.KEY_PASSWORD));

            	if(!userPass.equals(correctPass)){
            		//password is incorrect
            		//inform the user to retry
                	logInMsg.setText("password is incorrect, please try again");
                	logInMsg.setVisibility(View.VISIBLE);
                	...
                	logInMsg.setText("user name and password correct, loading...");
                	logInMsg.setVisibility(View.VISIBLE);
    				startActivity(new Intent(WishListLoginReal.this, WishList.class));
